...
public class SendFileThread implements Runnable{
	...
	private int sendType;	//0为发送给所有人，1为发送给个人
	private int encType;	//0为加密，1不加密
	
	private BufferedInputStream bis;
	private BufferedOutputStream bos;
	
	private CipherInputStream cipherInputStream;
	public SendFileThread(Message mess,int snedType,int encType){
		this.mess = mess;
		this.sendType = snedType;
	...
	public void run() {
		...
		//	while ((len = bis.read(bys)) != -1) {
			...
			//	while ((len = cipherInputStream.read(bys)) != -1) {
				...
				if(sendType==0){
					//发送给所有人
					SendFileToAll2(Name+""+mess.getContent());
				}else if(sendType==1){
					//发送给个人
					SendFileToPerson2(Name+""+mess.getContent());
				}
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
	 ...
	 * 不加密发送给个人
	 */
	public void SendFileToPerson2(String FileName){
		Socket s1 = null;;
		try {
			//根据获得者取得服务器端与客户端通信的线程
			ServerConClient sccc = ServerThreadCollection.getServerContinueConnetClient(mess.getGetter());
	 ...
	 * 不加密发送文件给所有人
	 */
	public void SendFileToAll2(String FileName){
		//获得在线用户
		String string = ServerThreadCollection.GetOnline();
		String[] strings = string.split(" ");
		String Name = null;
